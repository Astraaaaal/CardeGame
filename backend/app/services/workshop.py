...
async def tap(session: AsyncSession, user: User, count: int) -> dict:
    ...
    if row.workshop_gauges_today >= ws["gauges_per_day"]:
        accepted = 0
    row.workshop_last_at = now
    row.workshop_taps += accepted

    reward = {"coins": 0, "dust": 0, "gauges": 0, "boosters": 0}
    while row.workshop_taps >= ws["taps_per_gauge"] and row.workshop_gauges_today < ws["gauges_per_day"]:
    ...
    await session.commit()
    return {
        **_state(row, cfg), "accepted": accepted, "reward": reward,
